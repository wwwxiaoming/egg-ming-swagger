...
class eggSwagger {
  constructor(router, header,app) {
    ...
    // 判断一下路由是否是数组，如果不是数组就把对象放进数组里面
    if (router.constructor === Object) {
      this.router = [];
      this.router.push(router)
    } else if (router.constructor === Array) {
      this.router = router;
    }
  ...
  initSwagger() {
    writerSwagger(this.json, this.router, this.app, this.header,this.appFile);
  }
}

// 写入swagger.json文件
function writerSwagger(json, router, app, header,appFile) {
  initTags(router, json);
  writerRouter(router, json, app);
  if (header) {
    initHeader(json, header);
  }
  writerFile(json,appFile);
}
  ...
  for (const routers of router) {
    for (const paths in routers.routers) {
      ...
      if (routers.routers[paths].hasOwnProperty('middleware')) {
        if (routers.routers[paths].middleware.constructor === Array) {
...
// 写入文件
function writerFile(json,appFile) {
  const file_path = path.join(appFile, './public/swagger/swagger.json');
